...
export default function AIAssistantWidget({ locale }: { locale: Locale }) {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      text: t(locale, "aiGreeting"),
    },
  ]);
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: "assistant", text: t(locale, "aiError") }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      {open && (
        <div className="mb-3 flex h-[28rem] w-80 flex-col overflow-hidden rounded-2xl border border-black/10 bg-white shadow-2xl sm:w-96">
          <div className="flex items-center justify-between bg-brand px-4 py-3 text-white">
            <span className="font-medium">{t(locale, "aiWidgetTitle")}</span>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-3">
                  ...
                  {m.recommendations && m.recommendations.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {m.recommendations.map((r) => (
                        ...
                        >
                          → {pickLocalized(r.service.name, r.service.nameKk, locale)}
                        </Link>
                      ))}
                    </div>
                  )}
            ...
            {loading && <div className="text-xs text-slate-400">{t(locale, "aiTyping")}</div>}
            <div ref={endRef} />
          </div>
          <div className="flex items-center gap-2 border-t border-black/10 p-2">
            <input
              ...
              placeholder={t(locale, "aiInputPlaceholder")}
      ...
      <button
        ...
        aria-label={t(locale, "aiWidgetTitle")}
      >
