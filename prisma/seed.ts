...
async function main() {
  ...
  await prisma.lookup.create({
    data: {
      ...
      items: {
        create: [
          { value: "gondola", label: "Полувагон", labelKk: "Жартылай вагон", order: 1 },
          { value: "tank", label: "Вагон-цистерна", labelKk: "Цистерна-вагон", order: 2 },
          { value: "covered", label: "Крытый вагон", labelKk: "Жабық вагон", order: 3 },
          { value: "platform", label: "Платформа", labelKk: "Платформа", order: 4 },
          { value: "hopper", label: "Хоппер", labelKk: "Хоппер", order: 5 },
        ],
      },
    },
  });
  ...
  await prisma.lookup.create({
    data: {
      ...
      items: {
        create: [
          { value: "cattle_dairy", label: "КРС молочного направления", labelKk: "Сүтті бағыттағы ІРМ", order: 1 },
          { value: "cattle_beef", label: "КРС мясного направления", labelKk: "Етті бағыттағы ІРМ", order: 2 },
          { value: "sheep_goat", label: "Овцы и козы", labelKk: "Қой мен ешкі", order: 3 },
          { value: "horses", label: "Лошади", labelKk: "Жылқы", order: 4 },
          { value: "poultry", label: "Птица", labelKk: "Құс", order: 5 },
        ],
      },
    },
  });
  ...
  const wagonService = await prisma.service.create({
    data: {
      ...
      nameKk: "Вагондарды лизингке сатып алу",
      shortDescription: "Финансовый лизинг грузовых вагонов для промышленных и логистических компаний",
      shortDescriptionKk: "Өнеркәсіп және логистика компанияларына арналған жүк вагондарының қаржы лизингі",
      fullDescription:
      ...
      fullDescriptionKk:
        "Қызмет өз жүк тасымалдары үшін жылжымалы құрам (жартылай вагондар, цистерналар, платформалар және т.б.) қажет ететін қазақстандық компанияларға арналған. ҚДБ-Лизинг вагондарды сатып алуды 10 жылға дейінгі мерзімге қаржыландырады, сыйақы мөлшерлемесіне мемлекеттік қолдау көрсету мүмкіндігімен.",
      category: "Лизинг",
      icon: "TrainFront",
      status: "PUBLISHED",
                    ...
                    create: [
                      {
                        ...
                        key: "applicant_bin",
                        label: "БИН компании",
                        labelKk: "Компанияның БСН",
                        hint: "Введите БИН — наименование организации подтянется автоматически",
                        hintKk: "БСН енгізіңіз — ұйым атауы автоматты түрде толтырылады",
                        type: FieldType.TEXT,
                        required: true,
                  ...
                  fields: {
                    create: [
                      ...
                      },
                      {
                        ...
                        labelKk: "1 вагонның құны, теңге",
                        hint: "Согласно коммерческому предложению поставщика",
                        hintKk: "Жеткізушінің коммерциялық ұсынысына сәйкес",
                        type: FieldType.NUMBER,
                        required: true,
                      ...
                      },
                      {
                        ...
                        labelKk: "Жалпы құны, теңге",
                        hint: "Рассчитывается автоматически: количество × цена",
                        hintKk: "Автоматты түрде есептеледі: саны × баға",
  ...
  const livestockService = await prisma.service.create({
    data: {
      ...
      nameKk: "Мал шаруашылығын субсидиялау",
      shortDescription: "Возмещение части затрат на приобретение и содержание племенного скота",
      shortDescriptionKk: "Асыл тұқымды мал сатып алу мен ұстауға жұмсалған шығындардың бір бөлігін өтеу",
      fullDescription:
      ...
      fullDescriptionKk:
        "Мал шаруашылығы бағытын дамытып жатқан ауыл шаруашылығы өнімін өндірушілерге арналған мемлекеттік субсидия. Субсидия мөлшерлемесі мал түрі мен басының санына байланысты.",
              ...
              create: [
                {
                  ...
                  fields: {
                    create: [
                      ...
                      { order: 2, key: "farm_name", label: "Наименование хозяйства", labelKk: "Шаруашылықтың атауы", type: FieldType.TEXT, required: true },
                      { order: 3, key: "region", label: "Регион", labelKk: "Өңір", type: FieldType.LOOKUP, required: true, lookupCode: "regions" },
                    ],
                  },
                },
                {
                    ...
                    create: [
                      {
                        ...
                        labelKk: "Мал шаруашылығының түрі",
                        type: FieldType.LOOKUP,
                        required: true,
                      ...
                      },
                      {
                        ...
                        labelKk: "1 басқа субсидиялау мөлшерлемесі, теңге",
                        hint: "Согласно действующим нормативам МСХ РК",
                        hintKk: "ҚР АШМ қолданыстағы нормативтеріне сәйкес",
                        type: FieldType.NUMBER,
                        required: true,
                      },
                      {
                        ...
                        labelKk: "Есептелген субсидия сомасы, теңге",
                        type: FieldType.CALCULATED,
